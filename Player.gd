...
func _fire_starboard():
	...
	ball.transform = global_transform
	ball.transform.origin += Vector3.UP * 2
	var direction = transform.basis.z.normalized().rotated(transform.basis.y.normalized(), - PI / 2)
	ball.apply_central_impulse(direction * 25 + velocity)
